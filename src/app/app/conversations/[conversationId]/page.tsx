...
export default async function ConversationDetail({
  ...
  const [convo] = await d
    .select()
    .from(conversations)
    .where(and(eq(conversations.id, conversationId), eq(conversations.businessId, business.id)))
    .limit(1);
  if (!convo) notFound();
  ...
  const thread = await d
    .select()
    .from(messages)
    .where(and(eq(messages.conversationId, conversationId), eq(messages.businessId, business.id)))
    ...
    <main className="space-y-5">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--ink-soft)]">
            {convo.channel} · <Badge tone={convo.status === "handoff" ? "warn" : convo.status === "closed" ? "neutral" : "ok"}>{convo.status}</Badge>
          </p>
        </div>
        <Link href="/app/conversations" className="text-sm text-sky-600 hover:underline">
          ← All conversations
        </Link>
      </header>
      ...
      {order && (order.active || order.completed) && (
        <Card>
          ...
          <p className="mt-1 text-xs text-[var(--ink-soft)]">Exactly what the bot currently thinks it knows about this order.</p>
          <div className="mt-2 grid gap-1 text-sm sm:grid-cols-2">
            <span>Name: <strong>{order.customerName || "—"}</strong></span>
            <span>Phone: <strong>{order.phone || "—"}</strong></span>
            <span>Street &amp; number: <strong>{order.streetAndNumber || "—"}</strong></span>
            <span>City: <strong>{order.city || "—"}</strong></span>
            <span>Postal code: <strong>{order.postalCode || "—"}</strong></span>
            <span>Ordering: <strong>{order.productText || "—"}</strong></span>
          </div>
          <div className="mt-3">
            {order.completed ? (
              <Badge tone="ok">Order saved</Badge>
            ) : order.active ? (
              <Badge tone="warn">In progress — still missing: {missing.length ? missing.map((f) => orderFieldLabel(f, "en")).join(", ") : "nothing"}</Badge>
            ) : (
      ...
      <Card>
        <h2 className="font-semibold">Messages ({thread.length})</h2>
        {thread.length === 0 ? (
          <p className="mt-2 text-sm text-[var(--ink-soft)]">No messages.</p>
        ) : (
            ...
            {thread.map((m) => (
              <div key={m.id} className={`flex ${m.direction === "outbound" ? "justify-end" : "justify-start"}`}>
                ...
                >
                  ...
                  <p className="whitespace-pre-wrap">{m.text || <span className="italic text-[var(--ink-soft)]">(empty message)</span>}</p>
