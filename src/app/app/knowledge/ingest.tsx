...
export function IngestPanel({ businessId }: { businessId: string }) {
  ...
  return (
    <Card>
      <h2 className="font-semibold">Import old chats / notes</h2>
      <p className="mt-1 text-xs text-[var(--ink-soft)]">
        Paste a chat export, CSV or notes. Personal data (emails, phones, order/tracking numbers, addresses, marked names) is
        redacted before anything is stored. We extract FAQ candidates automatically. (PDF/DOCX upload is coming — paste text for
        now.)
      </p>
      <form action={formAction} className="mt-3 space-y-2">
        ...
        <Textarea name="content" rows={6} required placeholder="Paste transcript / CSV / notes here…" />
