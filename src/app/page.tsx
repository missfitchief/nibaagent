...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ lang?: string }> }): Promise<Metadata> {
  ...
  const title = "NibaChat Agent — " + (locale === "en" ? "AI agent for Instagram & Facebook messages" : "AI agent za Instagram i Facebook poruke");
  const languages: Record<string, string> = {};
  for (const l of LOCALES) languages[HREFLANG[l]] = `${BASE}/?lang=${l}`;
  return {
    title,
    description: t.hero.sub,
    alternates: { canonical: `${BASE}/?lang=${locale}`, languages },
    openGraph: { title, description: t.hero.sub, url: `${BASE}/?lang=${locale}`, locale: HREFLANG[locale].replace("-", "_"), type: "website" }
  };
}

export default async function LandingPage({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  const sp = await searchParams;
  const locale: Locale = await getLocale(isLocale(sp.lang) ? sp.lang : undefined);
  const t = getDict(locale);

  // Structured data (Task 4): Organization + SoftwareApplication + FAQPage.
  const jsonLd = {
    "@context": "https://schema.org",
    "@graph": [
      { "@type": "Organization", name: "NibaChat Agent", url: BASE, logo: `${BASE}/icon.svg`, sameAs: [] as string[] },
      {
