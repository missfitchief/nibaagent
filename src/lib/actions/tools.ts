...
export async function testBotAction(_prev: TestState, formData: FormData): Promise<TestState> {
  ...
  try {
    ...
    return { result: { ...result, tokenEstimate: 0, costEstimateEur: 0 } };
  } catch (err) {
...
export async function analyzeOldChatsAction(formData: FormData): Promise<void> {
  ...
  if (data.error) {
    ...
    const cost = estimateCostEur("gpt-4o-mini", data.usage?.prompt_tokens ?? 0, data.usage?.completion_tokens ?? 0);
