...
import { botSettings, businesses, eventLogs, metaConnections, products, unansweredQuestions } from "../src/lib/db/schema";
...
async function fill(businessId: string) {
  ...
  await db.insert(unansweredQuestions).values({ businessId, questionText: "koliko košta dostava?" });
}
...
describe("hard delete (purgeBusinessData)", () => {
  it("removes every tenant table for the target and leaves other tenants intact (FK-safe)", async () => {
    ...
    expect(await db.select().from(unansweredQuestions).where(eq(unansweredQuestions.businessId, a.business.id))).toHaveLength(0);
    ...
    expect(await db.select().from(unansweredQuestions).where(eq(unansweredQuestions.businessId, b.business.id))).toHaveLength(1);
  });
